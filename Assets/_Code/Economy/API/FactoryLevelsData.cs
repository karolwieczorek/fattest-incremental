...
namespace FattestInc.Economy.API {
    public class FactoryLevelsData : GoogleSpreadsheetScriptableObject, IFactoryLevelsData {
        ...
        [TableList] [SerializeField] List<LevelData> levelsList = new();
        ...
        [ShowInInspector] [PreviewField(128)] public Sprite Icon => icon;
        ...
        protected override void ProcessData(GoogleSheetJson data) {
            ...
            foreach (var sheetRow in data.values) {
                ...
                levelsList.Add(new LevelData {
                    level = level, cost = cost, value = value, time = time,
                });
        ...
        public ulong GetCostForNextLevel(int factoryLevel) {
            ...
            return FactoryLevelsUtils.GetCostForLevel(levelsList, nextLevel);
        }

        public ulong GetCostForLevel(int factoryLevel) {
            return FactoryLevelsUtils.GetCostForLevel(levelsList, factoryLevel);
        }

        public ulong GetValueForLevel(int factoryLevel) {
            if (factoryLevel == 0)
                return 0;
            return FactoryLevelsUtils.GetValueForLevel(levelsList, factoryLevel);
        }

        public int GetDurationForLevel(int factoryLevel) {
            if (factoryLevel == 0)
                return 0;
            return FactoryLevelsUtils.GetDurationForLevel(levelsList, factoryLevel);
        }
